...
def post_start(term, betting_start_time):  # 开始比赛
...
def post_end(term, betting_end_time, status=1):  # 结束比赛
    POST_end_url = '/end'
...
def post_result(term, betting_end_time, result_data=''):  # 发送赛果
...
def post_upload(term, img_path):  # 上传图片
...
def post_aiupload(term, ai_img_path):
...
def post_status(status=True):  # 发送开盘封盘状态，开盘了才能正常进行操作

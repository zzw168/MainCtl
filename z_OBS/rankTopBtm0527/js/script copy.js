...
var count = 0;
var positions = [];
var scoketUrl = "103.30.77.175"  //这是与服务器的域名或IP
var port = 9999
var isTest = 1
...
let pstNumArr=  [{idx:1,name:'A',time:'20:39'},
            ...
            $(_=>{
               pstNumArr.forEach(function(el,i) {
                ...
                <div class="time rank_label flex-box"><div style="display:none">${el.time}</div> </div> 
             </li>`
...
function toRunSortRank(socArr,isTime){
    ...
    $("ul.letters li").each(function(i) {
        console.log("sssdfff;",y,socArr[i]-1)
        $('.ranks'+(socArr[i]-1)).stop()
        // +(socArr[i]-1))
        // $(this).find('.ranks'+socArr[i]-1).stop()
        // $(this).find('.time').css("left",80+ballWin+'px')
        ...
        if( isTime&&inds+1==socArr.mc){
            $(this).find('.rank_label').eq(1).hide()
            $(this).find('.rank_label').eq(1).addClass('aniFont').find('div').text(socArr.data)
         
         
            //  window.desyTime =  setTimeout(_=>{
                $(this).find('.rank_label').eq(1).show()
                $(this).find('.rank_label').eq(1).find('div').show()

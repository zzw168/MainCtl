...
for i in range(1, 13):  # Assuming you have 6 sets of coordinates
    ...
    cv2.polylines(mask, [pts], isClosed=True, color=(0, 255, 255), thickness=6)
    masks[i] = mask
...
def reset_frame_size(integration_frame_array, target_width, target_height,config):
    ...
    for v in config['video_arr']:
        resized_images[int(v)] = False


    for i, item in integration_frame_array.items():

        if item is not False:
            resized_img = cv2.resize(item, (target_width, target_height))

            indexes_num_equals_2 = [key for key, value in config['imgSimilarList'].items() if value['num'] == int(i)]
            ...
            resized_images[int(i)] = resized_img
        else:
            resized_images[int(i)] = item
...
def setSaveImg(selfs, post_params, extra_param_config):
    ...
    if post_params.get('save_ball_num', False):
        extra_param_config['save_ball_num'] = post_params.get('save_ball_num')[0]
    else:
        extra_param_config['save_ball_num'] = ''
...
def is_camera_black_screen(frame, sample_step=20):
    # 直接使用图像的蓝色通道 (B通道)
    blue_channel = frame[:, 0, :]  # 0 表示B通道

    # 获取采样像素
    sample_pixels = blue_channel[::sample_step, ::sample_step]  # 每隔 sample_step 行列采样

    # 计算采样像素的平均亮度
    mean_brightness = np.mean(sample_pixels)


    # 判断是否黑屏
    threshold = 5
    return mean_brightness < threshold or int(mean_brightness) == 0
...
# 处理帧 操作
def processing_frames(cap_num, cap, ifsaveimg, config, image_queue, iftensor=False,gpu=0):
    global names,namesColor,addside,listsidepolygon
    ...
    indexes_num_equals_2 = [key for key, value in config['imgSimilarList'].items() if value['num'] == int(cap_num)]
    ...
    if indexes_num_equals_2:
        ...
        if iftensor == False:
            ...
            results = config['model'].predict(source=frame, show=False, verbose=False, conf=0.5, iou=0.45,
                                              imgsz=config['myimgsz'],device=f"{gpu}")
            qiu_array = []
            qiu_array1 = []
            if len(results) != 0:
            ...
            if len(results) != 0:  # 整合球的数据
                ...
                for r in results:
                    if int(r[5].item()) < 10:
                        ...
                        array1 = [r[0], r[1], r[2], r[3], r[5], names[r[4]], int(indexes_num_equals_1),1]
        ...
        # 判断是否保存截图
        if len(class_arr) > 0 or int(config['saveBackground']) == 1:

            # 使用 in 关键字判断是否包含子字符串
            if str(indexes_num_equals_1) in config['saveImgNum'] and int(config['saveImgRun']) == 1 and image_queue.unfinished_tasks < 1000 :

                if len(config['save_ball_num']) >= 1:
                    if any(f",{str(num)}," in f",{config['save_ball_num']}," for num in class_arr):
    ...
    # time222 = time.time()
    # print(str(int((time222 - time111) * 1000)) + "---------")
...
def processing_frames_end(res1, i, cap, cap_array, integration_frame_array, config):
    ...
    if isinstance(res1, str):
        ...
        if res1 is not False:
            integration_frame_array[i] = res1
        else:
...
def init_camera(cap_num_list, config):
    ...
    cap_array = cap_num_list.copy()

    for k, i in cap_num_list.items():
...
def set_cap(cap, config):  # 设置视频截图参数（不压缩图片，节省压缩过程时间）
    ...
    cap.set(cv2.CAP_PROP_BRIGHTNESS, 112)  # 设置亮度，范围一般在0到255之间
    cap.set(cv2.CAP_PROP_CONTRAST, 115)  # 设置对比度，范围一般在0到255之间
    cap.set(cv2.CAP_PROP_SATURATION, 91)  # 设置饱和度，范围一般在0到255之间
    cap.set(cv2.CAP_PROP_HUE, 10)  # 设置色调，范围一般在0到255之间
    cap.set(cv2.CAP_PROP_GAIN, 40)  # 设置增益，范围一般在0到255之间
    cap.set(cv2.CAP_PROP_EXPOSURE, -8)  # 设置曝光时间，通常曝光时间是负值，值越大曝光越长
    # 设置白平衡 U 和 V 值
    cap.set(cv2.CAP_PROP_WHITE_BALANCE_BLUE_U, 4000)  # 设置 U 值
    cap.set(cv2.CAP_PROP_SHARPNESS, 40) # 尝试设置摄像头的焦距
    focus_supported = cap.set(cv2.CAP_PROP_FOCUS, 120)  # 焦距范围可能取决于摄像头，一般是 0 到最大值
    # 尝试设置摄像头的变焦
